...
backtester/
    ...
    reports/
        __init__.py                     # (* Phase 2 *) REPORTS dict + re‑exports
        _base.py                        # (* Phase 2 *) BaseReport ABC
        equity_curve.py                 # (* Phase 2 *) EquityCurveReport
        trade_summary.py                # (* Phase 2 *) TradeSummaryReport
        metrics.py                      # (* Phase 2 *) MetricsReport
        periodic_metrics.py             # (* Phase 2 *) PeriodicMetricsReport
        hit_ratio.py                    # (* Phase 2 *) HitRatioReport
        drawdown_table.py               # (* Phase 2 *) DrawdownTableReport
...
- `reports`: a dictionary that defines the reports to generate.  
 ...
 - `missing_data_mode`: string, `'any'` (default), `'all'`, or `'per_leg'`. Applies to **all** reports in this run. To compare different missing‑data modes, call `generate()` again with a different value.
   ...
   - `'all'`: For every trade (including single‑leg trades), if any leg has genuine missing data on a day, the P&L of all legs in that trade is zeroed and deferred to the next valid trading day. This prevents one‑sided P&L moves from creating artificial drawdowns. Risk series (`*_ts`) are forward‑filled within the trade with the last known value on those days. Component P&L (`*_pnl`) follows the same deferral as the primary P&L. Legs from different trades are not affected by each other.
...
|-------------------|-------------|---------|------------------------------|
| `'equity_curve'` | Daily portfolio‑level equity series. | `include` | `'gross'` (cumulative gross P&L), `'cost'` (cumulative cost), `'net'` (cumulative net P&L), `'decomposition'` (daily P&L decomposition, if configured). If FX rates were supplied, the equity curve also includes the spot FX rate used for each currency conversion (e.g., `fx_USDJPY`). **When `capital` is provided:** `'gross_pct'`, `'cost_pct'`, `'net_pct'` — cumulative P&L expressed as a percentage of capital. If `'include'` is omitted, all groups are produced. |

**Phase 2 – portfolio‑level risk and component P&L**  
Once risk measures and component P&L are implemented, the equity curve can include daily portfolio‑level series for any aggregated measure that appears at the trade level in `trade_breakdown`. This includes net and gross risk, and net component P&L.  
**Gross measures at the portfolio level are not computed by summing the gross values of individual trades.** Instead, the correct method is: (1) for each underlying across all trades, compute the net risk time series (signed sum of all legs for that underlying); (2) for each date, take the absolute value of each underlying's net risk; (3) sum those absolute values across all underlyings. This avoids double‑counting that would occur if absolute values were taken at the trade level before underlying‑level netting.  
These series are controlled by the standard `include` mechanism.

| `'by_underlying'` | One sheet per underlying instrument. | `include` | `'include'` is a **dictionary** mapping metric groups to their sub‑report configs. Supported keys and their sub‑configs:<br>• `'equity_curve'`: `{'include': ['gross','cost','net','decomposition']}`<br>• `'trade_summary'`: `{'include': ['tags','gross_pnl','cost','net_pnl','underlying','holding_days']}`<br>• `'risk'`: `{'include': ['greeks']}`<br>• `'drawdown_table'`: `{'include': ['gross','net'], 'top_n': <int>}`<br>• `'hit_ratio'`: `{'include': ['gross','net'], 'timeframe': 'yearly'\|'monthly'}`<br>• `'metrics'`: `{'include': ['sharpe_gross', …], 'annualization': <int>}`<br>• `'periodic_metrics'`: `{'include': ['sharpe_gross', …], 'timeframe': 'yearly'\|'monthly', 'annualization': <int>}`<br>`'by_underlying'` does **not** accept a `'filter'` key directly. Wrap it in a report group to filter. |

| `'trade_summary'` | One row per trade with key attributes and P&L. | `include` | `'tags'`, `'gross_pnl'`, `'cost'`, `'net_pnl'`, `'underlying'` (comma‑separated tickers), `'holding_days'` (trading days alive, business days only). **When `capital` is provided:** `'gross_pnl_pct'`, `'cost_pct'`, `'net_pnl_pct'` — trade‑level P&L expressed as a percentage of capital. Trade ID, entry date, and exit date are always included regardless of `include`. If `include` is omitted, all groups are shown. |
...
- A `BaseMetricCalculator` ABC and registry (`backtester/metrics_registry.py`) provide pluggable metric calculators for sharpe, max_drawdown, return, return_pct, annualized_return, calmar, and hit_ratio. The shared helper `_compute_metrics_row` is used by `MetricsReport` and `PeriodicMetricsReport`. Capital‑dependent metrics (annualized_return, return_pct, max_drawdown_pct) are automatically excluded when no capital is set.
- A `BaseReport` ABC and registry (`backtester/reports/`) provide pluggable report classes for each standard report: equity_curve, trade_summary, metrics, periodic_metrics, hit_ratio, drawdown_table, and by_underlying. The `REPORTS` dict in `__init__.py` maps string names to concrete report classes.
- The `Summary` class owns only the shared state (leg data, trading days, capital, missing‑data mode) and exposes public caching helpers (`get_daily_series`, `get_cumulative_series`, `get_trade_totals`) to all registered reports. It dispatches report generation through the `REPORTS` registry with no hard‑coded `_build_*` methods.
- Adding a new metric or report requires only a new class and a registry entry; the `Summary`'s dispatch code never changes.
